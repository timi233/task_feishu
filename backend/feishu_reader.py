...
class FeishuBitableReader:
    ...
    def _get_tenant_access_token(self) -> Optional[str]:
        ...
        try:
            ...
            if result.get("code") == 0:
                ...
                print(f"✅ Successfully obtained tenant_access_token, expires at {time.ctime(self.token_expire_time)}")
                return self.access_token
            else:
            ...
            print(f"❌ Exception while getting tenant_access_token: {e}")
            return None
    ...
    def get_records(self, app_token: str, table_id: str, page_size: int = 100) -> List[Dict[str, Any]]:
        ...
        if not token:
            print("❌ Failed to get access token, cannot fetch records")
            return []
        ...
        while has_more:
            ...
            try:
                ...
                if result.get("code") == 0:
                    ...
                    print(f"📥 Fetched {len(items)} records, total so far: {len(records)}")
        ...
        print(f"✅ Finished fetching all records. Total: {len(records)}")
        return records
