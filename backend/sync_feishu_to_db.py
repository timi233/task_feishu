...
def sync_feishu_data_to_db():
    ...
    try:
        ...
        # 2. 从飞书获取原始数据（带重试）
        logger.info("[SYNC] Fetching raw task data from Feishu...")

        def fetch_task_records():
            """闭包函数用于重试"""
        ...
        if not raw_records:
            logger.warning("[SYNC] No raw data fetched from Feishu. Skipping task sync.")
        else:
            # 3. 处理原始数据
            logger.info("[SYNC] Processing fetched data...")
            ...
            logger.info(f"[SYNC] Task synchronization completed: {len(processed_tasks)} tasks saved")

        # === 第二部分: 同步工程师数据 ===
        engineer_count = sync_engineers_to_db()

        logger.info("[SYNC] Data synchronization completed successfully.")
        logger.info(f"[SYNC] Summary: Tasks synced, {engineer_count} engineers synced")
