...
async def local_login(request: Request):
    """Local fallback login for emergency administration."""
    try:
    ...
    expected_username = os.getenv("LOCAL_ADMIN_USERNAME", "admin")
    expected_password = os.getenv("LOCAL_ADMIN_PASSWORD", "admin@123")

    if username != expected_username or password != expected_password:
        logger.warning("Local admin login failed for username=%s", username)
        raise HTTPException(
    ...
    session_id = session_manager.create_session(
        {
            ...
            "refresh_token": None,
            "user_id": "local_admin",
            "identity_hub_id": None,
            "user_name": "admin",
            "user_email": "admin@local",
            "user_mobile": None,
            "user_permissions": LOCAL_ADMIN_PERMISSIONS,
            "user_roles": [LOCAL_ADMIN_ROLE],
            "authenticated": True,
            "auth_source": "local",
        }
    )
    ...
    logger.info("Local admin logged in: %s", username)
    return response


@router.get("/login")
async def login(response: Response, return_url: str = "/"):
    """登录端点
    ...
    try:
        ...
        session_id = session_manager.create_session(
            {"oauth_state": state, "return_url": return_url}
        )
...
async def callback(
    ...
    if error:
        ...
        frontend_url = os.getenv("FRONTEND_URL", "https://itcheck.purytech.cn")
        return RedirectResponse(
            url=f"{frontend_url}/?error={error}", status_code=status.HTTP_302_FOUND
        )
    ...
    logger.info(
        f"Callback received: session_id={session_id[:16] if session_id else 'None'}..., code={code[:16] if code else 'None'}..., state={state[:16] if state else 'None'}..."
    )
    ...
    if not session_id:
        ...
        logger.error(
            f"   Possible causes: 1) Cookie blocked by browser, 2) Container restarted, 3) SameSite policy"
        )
        # 返回更友好的错误页面
        frontend_url = os.getenv("FRONTEND_URL", "https://itcheck.purytech.cn")
        return RedirectResponse(
    ...
    if state != stored_state:
        ...
        logger.error(
            f"   Stored state:   {stored_state[:16] if stored_state else 'None'}..."
        )
    ...
    try:
        ...
        if is_identity_hub_user:
            ...
            with get_db_connection() as conn:
                ...
                if feishu_user:
                    ...
                    logger.info(
                        f"⚠️ Identity Hub用户 {user_name} ({user_id}) 已有同名飞书用户 ({actual_user_id})，使用飞书账号"
                    )
        ...
        try:
            ...
            logger.info(
                f"从本地数据库获取用户权限成功: {len(user_permissions)}个权限, {len(user_roles)}个角色"
            )
        except Exception as local_error:
            logger.warning(
                f"从本地数据库获取权限失败，尝试从Identity Hub获取: {local_error}"
            )
            ...
            try:
                ...
                logger.info(
                    f"从Identity Hub获取用户权限成功: {len(user_permissions)}个权限, {len(user_roles)}个角色"
                )
        ...
        session_manager.update_session(
            session_id,
            {
                ...
                "identity_hub_id": user_id
                if is_identity_hub_user
                else None,  # 保存原始Identity Hub ID
                "user_name": user_info["name"],
        ...
        logger.info(
            f"✅ User logged in: {user_info['name']} (actual_id={actual_user_id}, hub_id={user_id})"
        )
        ...
        try:
            if actual_user_id != user_id:
                ...
                logger.info(
                    f"✅ 使用飞书账号 {actual_user_id}，跳过同步Identity Hub用户 {user_id}"
                )
            else:
                ...
                logger.info(
                    f"✅ 已同步Identity Hub用户信息到本地: {user_info['name']} - {department_name}"
                )
        ...
        frontend_url = os.getenv("FRONTEND_URL", "https://itcheck.purytech.cn")
        return RedirectResponse(
            url=f"{frontend_url}/?error=oauth_failed", status_code=status.HTTP_302_FOUND
        )
