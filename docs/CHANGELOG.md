...
### 外勤数据源接入
- 新增 `FEISHU_FIELD_DISPATCH_TABLE_ID=tblC8B2jvybfbdVM`，`dispatch_sync.py` 现在会读取“外勤申请”表，按“外勤类型”动态映射到 `eisoo_dispatch/work_order`。
- 为外勤记录提供字段别名、`未登录时显示的人名` 兜底提交人，并在 `extra_data._source_table`、`_field_dispatch_type` 中标注来源。
- `dispatch_db.py` 引入 `_deduplicate_parallel_orders()`，以客户+时间去重外勤与其它派工的重复记录，统计/分页/API 均使用去重后的结果。

### 同步与运维
- 新增 `scripts/docker_sync_all.sh`，一键在 `docker-app-1` 内执行 `sync_once.py` 与 `dispatch_sync.sync_all()`，便于运维快速拉取最新任务/派工。
- 调整 docker 同步流程，确认容器数据目录 `/app/data/db/tasks.db` 中可直接验证新记录（示例：`202601120001`）。

### 前端展示优化
- `convertDispatchOrderToTask` 会把外勤任务的执行人默认回退到提交人，并把 `source_table` 一并下发，供隐私策略识别。
- `applyTaskPrivacy` 在受限视图下优先显示提交人，避免外勤记录出现大面积“未分配”。
- 编译后重新发布于 `docker-frontend-1`，对应 bundle `static/js/main.f91632a8.js`。
